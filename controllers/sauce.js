...
exports.createSauce = (req, res, next) => {
  ...
  const sauce = new Sauce({
    //create a new instance of the model Sauce
  ...
  sauce
    .save() //save the sauce in the database
    //creation of Promise
    .then(() =>
      res.status(201).json({
...
exports.getOneSauce = (req, res, next) => {
  Sauce.findOne({
    // definition with Params of the same id as the sauce requested
    _id: req.params.id,
  }) //creation of Promise
    .then((sauce) => {
      let modified = {
        ...sauce._doc,
        imageUrl: `${req.image_url}/${sauce._doc.imageUrl}`,
      }
      res.status(200).json(modified) //　the response of success code 200
    })
    .catch((error) => {
      res.status(404).json({
...
exports.getAllSauces = (req, res, next) => {
  Sauce.find() // collect the list of all sauces with 'find'
    ...
    .then((sauces) => {
      ...
      let modified = sauces.map((s) => ({
        ...s._doc,
        imageUrl: `${req.image_url}/${s._doc.imageUrl}`,
      }))
  ...
  switch (like) {
    ...
    case -1:
      Sauce.updateOne(
        ...
        .then(() => {
          res.status(200).json({ message: 'I do not like it' })
        })
        .catch((error) => res.status(400).json({ error }))
      break
